...
class Cars():
    ...
    def __init__(self, ini_pos, ini_velo,wall_len):
        ...
        self.tol = 0.05
        self.wall = wall_len
        self.pub = rospy.Publisher('car2_pos', Float32, queue_size=10, latch=True)
        ...
        subscriber_left = message_filters.Subscriber('car1_pos', Float32)
        subscriber_right= message_filters.Subscriber('car3_pos', Float32)
    ...
    def callback(self,sub_left, sub_right):
        self.car1_p = float(sub_left.data)
        self.car3_p = float(sub_right.data)
        ...
        if self.pos <= self.car1_p + 2.0*self.tol:
            self.velo = -1.0* self.velo
...
if __name__ == "__main__":
    ...
    car2 = Cars(initial_pos, initial_velocity,wall_len)
    rospy.spin()
